...
class Agent: 
    ...
    # If lenght of the memory exceeds BATCH_SIZE we can train long memory more efficiently
    def trainLongMemory(self):
        if len(self.memory) > BATCH_SIZE: 
            # Take random sample from memory as a tuple
            miniSample = random.sample(self.memory) 
        else:
    ...
    def getAction(self, state):
        ...
        # Otherwise choose the move based on model
            ...
            # Get the prediction
            prediction = self.model.predict(inputState) 
...
def train():
    ...
    while True:
        ...
        reward, gameOver, score = game.takeAction()
        ...
        if gameOver:
            ...
            print('Game:', agent.gamesCounter, 'Score:', score, 'Record:', record)
            # TODO: plotting
